...
def main():
    ...
    [data, labels] = utils.load_data(args.src, args.input)
    
    for model in args.models.split():
        imgs = get_imgs.encode_data(data, args.algorithm, args.iscumm, args.verbose, args.chunk)/255.0
        ...
        if args.predict == 'SoC':
            locals()[str(model)+"result"] = locals()[str(model)+"result"]*100.0
    ...
    for idx in range(len(args.models.split())):
        ...
        plt.fill_between(range(len(labels)), labels_lower, labels_upper, color='red', alpha=0.3)
